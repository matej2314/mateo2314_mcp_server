...
export function toStrList(value) {
    if (value == null)
        return [];
    if (Array.isArray(value))
        return value.map((x) => norm(String(x))).filter(Boolean);
...
export function yearFromUnknown(value) {
    if (value == null)
        return undefined;
...
export function yearFromDateField(value) {
    if (value == null)
        return undefined;
    const s = String(value).trim();
    if (/^(19|20)\d{2}-\d{2}-\d{2}/.test(s))
        return parseInt(s.slice(0, 4), 10);
    return yearFromUnknown(value);
}
function ongoingEndDate(value) {
    const s = norm(String(value ?? ''));
    return s === 'today' || s === 'present' || s === 'ongoing' || s === '';
}
export function matchesProject(data, filters) {
    const checks = [];
    ...
    if (filters.status)
        checks.push(st === norm(filters.status));
    if (filters.year != null)
        checks.push(yr === filters.year);
    if (filters.role) {
...
export function matchesSkill(data, filters) {
    const checks = [];
    ...
    if (filters.level)
        checks.push(lvl === norm(filters.level));
    if (filters.type)
        checks.push(typ === norm(filters.type));
...
export function matchesExperience(data, filters) {
    const checks = [];
...
export function matchesCourse(data, filters) {
    const checks = [];
    const cat = normKey(String(data.category ?? ''));
...
export function uniqueSorted(values) {
    return [...new Set([...values].map((v) => v.trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'en'));
}
