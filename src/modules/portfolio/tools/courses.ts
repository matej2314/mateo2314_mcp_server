  ...
  registerInstrumentedTool(
    ...
    async (args) => {
      ...
      try {
        ...
        const items = all
          .filter((x) =>
            matchesCourse(x.data as Record<string, unknown>, filters),
          )
          .map(({ id, data }) => ({ id, data }));
  ...
  registerInstrumentedTool(
    ...
    async () => {
      ...
      try {
        ...
        const fromFiles = all
          .map((x) =>
            String((x.data as Record<string, unknown>).category ?? ""),
          )
          .filter(Boolean);
  ...
  registerInstrumentedTool(
    ...
    async () => {
      ...
      try {
        ...
        const fromFiles = all
          .map((x) =>
            String((x.data as Record<string, unknown>).platform ?? ""),
          )
          .filter(Boolean);
