...
export function registerCoursesTools(server: McpServer, options: ToolOptions) {
	...
	server.registerTool(
		...
		async (args) => {
			...
			try {
				...
				const items = all
					.filter((x) => matchesCourse(x.data as Record<string, unknown>, filters))
					.map(({ id, data }) => ({ id, data }));
		...
		async () => {
			...
			try {
				...
				for (const { data } of all) {
					fromFiles.push(...toStrList((data as Record<string, unknown>).tags));
				}
	...
	server.registerTool(
		...
		async () => {
			...
			try {
				const all = await readAllFiles('courses');
				const fromFiles = all.map((x) => String((x.data as Record<string, unknown>).category ?? '')).filter(Boolean);
	...
	server.registerTool(
		...
		async () => {
			...
			try {
				const all = await readAllFiles('courses');
				const fromFiles = all.map((x) => String((x.data as Record<string, unknown>).platform ?? '')).filter(Boolean);
